...
class MqttBridge(
    ...
    fun publishTest() = publish(
        topic("test"),
        JSONObject().put("bridge", "casambi").put("version", "0.7.7").toString(),
        false
    )
    ...
    fun publishHacsDiagnostics(c: BridgeConfig = config) {
        ...
        publish(topic("diagnostics/bridge_version"), "0.7.7", true)
    ...
    fun publishDiscoveryForDiagnostics() {
        log("MQTT Diagnostics Discovery in v0.7.7 voruebergehend deaktiviert")
    }
    fun publishDiagnosticStates() {
        // v0.7.7: disabled to avoid MQTT publish storms on Android/Paho.
        return
    }
