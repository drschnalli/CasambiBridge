...
class MqttBridge(
    ...
    fun publishTest() = publish(
        topic("test"),
        JSONObject().put("bridge", "casambi").put("version", "0.8.4").toString(),
        false
    )
    ...
    fun publishHacsDiagnostics(c: BridgeConfig = config) {
        ...
        publish(topic("diagnostics/bridge_version"), "0.8.4", true)
    ...
    fun publishDiscoveryForDiagnostics() {
        log("MQTT Diagnostics Discovery in v0.8.4 voruebergehend deaktiviert")
    }
    fun publishDiagnosticStates() {
        // v0.8.4: disabled to avoid MQTT publish storms on Android/Paho.
        return
    }
