...
class MqttBridge(
    ...
    fun publishTest() = publish(
        topic("test"),
        JSONObject().put("bridge", "casambi").put("version", "0.7.8").toString(),
        false
    )
    ...
    fun publishHacsDiagnostics(c: BridgeConfig = config) {
        ...
        publish(topic("diagnostics/bridge_version"), "0.7.8", true)
    ...
    fun publishDiscoveryForDiagnostics() {
        log("MQTT Diagnostics Discovery in v0.7.8 voruebergehend deaktiviert")
    }
    fun publishDiagnosticStates() {
        // v0.7.8: disabled to avoid MQTT publish storms on Android/Paho.
        return
    }
