...
<section class='hero card wide glassHero'><h1>Scanners</h1><p class='muted'>Live Network Scan im Browser, Bluetooth/WiFi Scan mit Android-Bridge backend. Network Scan schreibt Zeile fuer Zeile ins Terminal.</p></section>
<section class='card wide scannerGrid'>
  <div class='scannerPanel'>
    ...
    <div class='controlRow'><button class='btn' onclick='startNetworkLive()'>START LIVE</button><button class='btn ghost' onclick='stopNetworkLive()'>STOP</button><button class='btn ghost' onclick='clearTerminal()'>CLEAR</button></div>
    <div class='progress'><span id='netProgress'></span></div>
    <pre id='scanTerminal' class='terminal'>Bereit.</pre>
  </div>
  <div class='scannerPanel'>
    <h2>WiFi Scanner</h2>
    <form class='toolForm' action='/scan-wifi'><label>Filter SSID/BSSID</label><input name='filter'><button class='btn'>SCAN WIFI</button></form>
    <p class='muted'>Hinweis: Android 8.1 kann WiFi ScanResults auch bei erlaubten Rechten leer zurueckgeben.</p>
  </div>
  <div class='scannerPanel'>
    <h2>Bluetooth Scanner</h2>
    <form class='toolForm' action='/scan-ble'><label>Filter Name/MAC</label><input name='filter'><button class='btn'>SCAN BLE</button></form>
  </div>
</section>
<script>
let scanStop=false;
function term(line){const el=document.getElementById('scanTerminal');el.textContent += '\n' + line;el.scrollTop=el.scrollHeight;}
function clearTerminal(){document.getElementById('scanTerminal').textContent='Bereit.';document.getElementById('netProgress').style.width='0%';}
function stopNetworkLive(){scanStop=true;term('[STOP] angefordert');}
function subnetBase(value){let raw=(value||'').trim(); if(!raw) return ''; return raw.split('/')[0].split('.').slice(0,3).join('.');}
async function startNetworkLive(){scanStop=false;clearTerminal();const range=document.getElementById('netRange').value;const ports=document.getElementById('netPorts').value||'known';let base=subnetBase(range);term('[NETWORK] Live scan gestartet ports='+ports);for(let i=1;i<=254;i++){if(scanStop)break;let url='/api/probe-host?i='+i+'&ports='+encodeURIComponent(ports);if(base)url+='&base='+encodeURIComponent(base);try{const r=await fetch(url,{cache:'no-store'});const j=await r.json();document.getElementById('netProgress').style.width=Math.round(i/254*100)+'%';if(j.visible){term(j.line);}else if(i%25===0){term('[progress] '+i+'/254');}}catch(e){term('[error] '+e);}await new Promise(res=>setTimeout(res,12));}term(scanStop?'[NETWORK] gestoppt':'[NETWORK] fertig');}
</script>
""")
    }
    private fun probeHostJson(params: Map<String,String>): String {
        ...
        val reachable = if (ports.isEmpty()) runCatching { addr.isReachable(140) }.getOrDefault(false) else open.isNotEmpty() || runCatching { addr.isReachable(80) }.getOrDefault(false)
        ...
        val line = "$ip  name=${if (name == ip) "-" else name}  mac=$mac  ports=$portText"
        return JSONObject().put("ip", ip).put("visible", reachable).put("name", if (name == ip) "-" else name).put("mac", mac).put("ports", portText).put("line", line).toString()
    }
...
<div class='hero'>
  ...
  <div class='sub'>${esc(c.casambiNetworkName.ifBlank { "Bridge Control Center" })} - powered by Sambesi - v0.8.2</div>
...
<div class='grid'>
  ...
  <section class='card'><h2>Tools</h2><div class='controlRow'><a class='btn ghost' href='/fetch-api'>API Fetch</a><a class='btn ghost' href='/dashboard-export'>MQTT YAML schreiben</a><a class='btn ghost' href='/dashboard-yaml'>MQTT YAML anzeigen</a><a class='btn ghost' href='/dashboard-direct-export'>Direct YAML schreiben</a><a class='btn ghost' href='/dashboard-direct-yaml'>Direct YAML anzeigen</a><a class='btn ghost' href='/files'>SMB Browser</a><a class='btn ghost' href='/logs'>Live Log</a><a class='btn ghost' href='/scanners'>Scanners</a><a class='btn ghost' href='/backup'>Backup SMB</a><a class='btn ghost' href='/toggle-ws'>WebSocket ${if (c.webSocketLiveEnabled) "AUS" else "EIN"}</a></div></section>
...
Web Port=${c.webInterfacePort}</pre></section>
""")
    }
