...
<section class='card wide scannerGrid'>
  ...
  <div class='scannerPanel'>
    ...
    <pre id='wifiTerminal' class='terminal smallTerminal'>Bereit. Android 8.1 kann WiFi ScanResults leer liefern.</pre><div id='wifiCards' class='scanCards'></div>
  </div>
  <div class='scannerPanel'>
    <h2>Live Bluetooth Scanner</h2>
    <label>Filter Name/MAC</label><input id='bleFilter'>
...
function clearTerminal(id,progress){document.getElementById(id).textContent='Bereit.';document.getElementById(progress).style.width='0%'; if(id==='bleTerminal'){seenBle.clear();document.getElementById('bleCards').innerHTML='';} if(id==='wifiTerminal'){seenWifi.clear();document.getElementById('wifiCards').innerHTML='';} if(id==='scanTerminal'){document.getElementById('networkCards').innerHTML='';}}
...
function addBleCard(line){const name=val(line,'name'), mac=val(line,'mac');const sig=signalParts(line);const cand=String(line).indexOf('Casambi candidate')>=0;const card='<div class="scanCard bleCard"><div class="cardTop"><b>🔵 '+html(name)+'</b>'+(cand?'<span class="badge secWpa3">🌴 Casambi</span>':'')+'</div><div class="meta">MAC '+html(mac)+'</div><div class="signal"><span>'+html(sig.dbm)+' dBm · '+html(sig.q)+'</span><i>'+html(sig.bar)+'</i></div></div>';document.getElementById('bleCards').insertAdjacentHTML('beforeend',card);}
function addNetCard(line){const host=val(line,'host'), name=val(line,'name'), ping=val(line,'ping'), mac=val(line,'mac');const ports=(String(line).split('ports=')[1]||'-');const card='<div class="scanCard netCard"><div class="cardTop"><b>🟢 '+html(host)+'</b><span class="badge secWpa2">'+html(ping)+'</span></div><div class="meta">Name: '+html(name)+' · MAC: '+html(mac)+'</div><div class="features">Ports: '+html(ports).replace(/,/g,' · ')+'</div></div>';document.getElementById('networkCards').insertAdjacentHTML('beforeend',card);}
...
async function startBleLive(){bleStop=false;clearTerminal('bleTerminal','bleProgress');term('bleTerminal','[BLE] Live Scan gestartet');for(let round=1;round<=8;round++){if(bleStop)break;try{const f=document.getElementById('bleFilter').value||'';const r=await fetch('/api/scan-ble-live?filter='+encodeURIComponent(f)+'&round='+round,{cache:'no-store'});const j=await r.json();document.getElementById('bleProgress').style.width=Math.round(round/8*100)+'%';(j.rows||[]).forEach(line=>{if(!seenBle.has(line)){seenBle.add(line);term('bleTerminal',line);addBleCard(line);}});term('bleTerminal','[BLE] round '+round+' results='+(j.rows||[]).length);}catch(e){term('bleTerminal','[BLE error] '+e);}await new Promise(res=>setTimeout(res,220));}term('bleTerminal',bleStop?'[BLE] gestoppt':'[BLE] fertig');saveHistory('Bluetooth',document.getElementById('bleTerminal').textContent);}
...
<div class='hero'>
  ...
  <div class='sub'>${esc(c.casambiNetworkName.ifBlank { "Bridge Control Center" })} - powered by Sambesi - v0.9.1</div>
...
""")
    ...
    private fun scanHistoryPage(): String = page("Scan History", """
<section class='hero card wide glassHero'><h1>Scan History</h1><p class='muted'>Die letzten 10 Web-Scans als durchsuchbarer Baum. Filter wirkt live auf Typ, Datum und Inhalt.</p></section>
<section class='card wide historyTools'><input id='historyFilter' placeholder='Filter: wifi, bluetooth, 192.168, Schnuffel, WPA2...' oninput='loadHistory()'><button class='btn ghost' onclick='clearHistory()'>CLEAR HISTORY</button></section>
...
function lineCard(line){const cls=classifyLine(line);return '<div class="histLine '+cls+'">'+escHtml(line)+'</div>';}
function clearHistory(){localStorage.scanHistory='[]';loadHistory();}
function loadHistory(){const box=document.getElementById('historyBox');const f=(document.getElementById('historyFilter')?.value||'').toLowerCase();let arr=[];try{arr=JSON.parse(localStorage.scanHistory||'[]')}catch(e){};arr=arr.filter(function(h){return !f || (String(h.type)+' '+String(h.time)+' '+String(h.text)).toLowerCase().indexOf(f)>=0;});if(!arr.length){box.textContent='Keine passenden Scan-Eintraege gefunden.';return;}box.innerHTML=arr.map(function(h){const lines=String(h.text||'').split('\n').filter(Boolean);const wifi=lines.filter(x=>x.indexOf('ssid=')>=0).length;const net=lines.filter(x=>x.indexOf('host=')>=0).length;const ble=lines.filter(x=>x.indexOf('mac=')>=0&&x.indexOf('signal=')>=0).length;const body=lines.map(lineCard).join('');return "<details class='historyItem' open><summary><span>"+escHtml(h.time)+"</span><b>"+escHtml(h.type)+"</b><em>"+lines.length+" Zeilen · WiFi "+wifi+" · Hosts "+net+" · BLE "+ble+"</em></summary><div class='historyTree'>"+body+"</div></details>";}).join('');}
loadHistory();
</script>
""")
