...
object WebControlServer {
    ...
    private fun handle(path: String): Pair<String, String> {
        ...
        if (route == "/api/export-scan-smb") return "application/json; charset=utf-8" to exportScanTextToSmbJson(params)
        if (route == "/api/export-b64-smb") return "application/json; charset=utf-8" to exportBase64ToSmbJson(params)
    ...
    private fun safeExportName(raw: String, fallback: String): String = raw.replace(Regex("[^A-Za-z0-9_.-]"), "_").ifBlank { fallback }
    private fun exportScanTextToSmbJson(params: Map<String,String>): String {
        val ctx = appContext ?: return JSONObject().put("ok", false).put("error", "no context").toString()
        val c = ConfigStore.load(ctx)
        if (c.smbServer.isBlank() || c.smbShare.isBlank()) return JSONObject().put("ok", false).put("error", "SMB nicht konfiguriert").toString()
        val type = safeExportName(params["type"] ?: "scan", "scan")
        ...
        return try {
            ...
            val url = dir + name
            SmbFileOutputStream(SmbFile(url, smbCtx), false).use { it.write(text.toByteArray(Charsets.UTF_8)) }
            LogBus.log("Web Scan TXT SMB Export gespeichert: $url")
            JSONObject().put("ok", true).put("file", name).put("url", url).toString()
        } catch (t: Throwable) {
            JSONObject().put("ok", false).put("error", t.message ?: t.javaClass.simpleName).toString()
        }
    }
    private fun exportBase64ToSmbJson(params: Map<String,String>): String {
        val ctx = appContext ?: return JSONObject().put("ok", false).put("error", "no context").toString()
        val c = ConfigStore.load(ctx)
        if (c.smbServer.isBlank() || c.smbShare.isBlank()) return JSONObject().put("ok", false).put("error", "SMB nicht konfiguriert").toString()
        val name = safeExportName(params["name"] ?: "casambi_jungle_scan.xlsx", "casambi_jungle_scan.xlsx")
        val b64 = params["data"] ?: ""
        if (b64.isBlank()) return JSONObject().put("ok", false).put("error", "Keine XLSX Daten").toString()
        return try {
            ...
            SmbFile(dir, smbCtx).use { if (!it.exists()) it.mkdirs() }
            val url = dir + name
            SmbFileOutputStream(SmbFile(url, smbCtx), false).use { it.write(bytes) }
            LogBus.log("Web Scan XLSX SMB Export gespeichert: $url")
            JSONObject().put("ok", true).put("file", name).put("url", url).toString()
        } catch (t: Throwable) {
            JSONObject().put("ok", false).put("error", t.message ?: t.javaClass.simpleName).toString()
        }
...
<section class='card wide scannerGrid'>
  <div class='scannerPanel'>
    ...
    <div class='controlRow'><button class='btn' onclick='startNetworkLive()'>START LIVE</button><button class='btn ghost' onclick='stopNetworkLive()'>STOP</button><button class='btn ghost' onclick='clearTerminal("scanTerminal","netProgress")'>CLEAR</button><button class='btn ghost' onclick='exportTerminalTxtSmb("Network","scanTerminal")'>TXT SMB</button><button class='btn ghost' onclick='exportTerminalXlsxSmb("Network","scanTerminal")'>XLSX SMB</button></div>
  ...
  <div class='scannerPanel'>
    ...
    <div class='controlRow'><button class='btn' onclick='startWifiLive()'>START WIFI LIVE</button><button class='btn ghost' onclick='stopWifiLive()'>STOP</button><button class='btn ghost' onclick='clearTerminal("wifiTerminal","wifiProgress")'>CLEAR</button><button class='btn ghost' onclick='exportTerminalTxtSmb("WiFi","wifiTerminal")'>TXT SMB</button><button class='btn ghost' onclick='exportTerminalXlsxSmb("WiFi","wifiTerminal")'>XLSX SMB</button></div>
  ...
  <div class='scannerPanel'>
    ...
    <div class='controlRow'><button class='btn' onclick='startBleLive()'>START BLE LIVE</button><button class='btn ghost' onclick='stopBleLive()'>STOP</button><button class='btn ghost' onclick='clearTerminal("bleTerminal","bleProgress")'>CLEAR</button><button class='btn ghost' onclick='exportTerminalTxtSmb("Bluetooth","bleTerminal")'>TXT SMB</button><button class='btn ghost' onclick='exportTerminalXlsxSmb("Bluetooth","bleTerminal")'>XLSX SMB</button></div>
...
function xEsc(v){return String(v||'').replace(/[\u0000-\u0008\u000B\u000C\u000E-\u001F]/g,'').replace(/[&<>]/g,function(c){return {'&':'&amp;','<':'&lt;','>':'&gt;'}[c];});}
function xVal(line,key){let m=String(line).match(new RegExp(key+'=([^ ]+(?: [^ ]+)?)(?=  |$)'));return m?m[1]:'-';}
function xSig(line){let m=String(line).match(/signal=(-?\d+) dBm/);return m?m[1]:'';}
function xQual(line){let m=String(line).match(/signal=-?\d+ dBm ([A-Za-z ]+) [█░]+/);return m?m[1].trim():'';}
function xRows(type,text){return String(text||'').split('\n').filter(Boolean).filter(function(l){return l.indexOf('ssid=')>=0||l.indexOf('host=')>=0||(l.indexOf('mac=')>=0&&l.indexOf('signal=')>=0);}).map(function(l){let k=l.indexOf('ssid=')>=0?'WiFi':(l.indexOf('host=')>=0?'Network':'Bluetooth');return [k,k==='WiFi'?xVal(l,'ssid'):(k==='Network'?xVal(l,'name'):xVal(l,'name')),k==='Network'?xVal(l,'host'):'',k==='WiFi'?xVal(l,'bssid'):(k==='Bluetooth'?xVal(l,'mac'):xVal(l,'mac')),xSig(l),xQual(l),xVal(l,'sec'),xVal(l,'band'),xVal(l,'ch'),xVal(l,'freq'),xVal(l,'ping'),String(l).split('ports=')[1]||'',l.toLowerCase().indexOf('casambi')>=0?'yes':'',l];});}
function colName(n){let s='';while(n>0){let m=(n-1)%26;s=String.fromCharCode(65+m)+s;n=Math.floor((n-1)/26);}return s;}
function sheetXml(type,text){let cols=['Type','Name','IP','Address','Signal_dBm','Quality','Security','Band','Channel','Frequency','Ping','Ports','Casambi','Raw'];let rows=xRows(type,text);let all=[['CASAMBI JUNGLE SCAN REPORT'],['powered by Sambesi',type,new Date().toLocaleString(),'Rows '+rows.length],[],cols].concat(rows);let xml='<?xml version="1.0" encoding="UTF-8" standalone="yes"?><worksheet xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main" xmlns:r="http://schemas.openxmlformats.org/officeDocument/2006/relationships"><dimension ref="A1:N'+all.length+'"/><sheetViews><sheetView workbookViewId="0"><pane ySplit="4" topLeftCell="A5" activePane="bottomLeft" state="frozen"/></sheetView></sheetViews><sheetFormatPr defaultRowHeight="15"/><cols>';for(let i=1;i<=14;i++){let w=i===14?80:(i===2||i===4?24:14);xml+='<col min="'+i+'" max="'+i+'" width="'+w+'" customWidth="1"/>';}xml+='</cols><sheetData>';all.forEach(function(r,ri){xml+='<row r="'+(ri+1)+'">';r.forEach(function(v,ci){let style=ri===0?1:(ri===1?2:(ri===3?3:0));xml+='<c r="'+colName(ci+1)+(ri+1)+'" t="inlineStr"'+(style?' s="'+style+'"':'')+'><is><t>'+xEsc(v)+'</t></is></c>';});xml+='</row>';});xml+='</sheetData><mergeCells count="2"><mergeCell ref="A1:N1"/><mergeCell ref="A2:N2"/></mergeCells><tableParts count="1"><tablePart r:id="rId1"/></tableParts></worksheet>';return xml;}
function tableXml(rows){let ref='A4:N'+(rows+4);let cols=['Type','Name','IP','Address','Signal_dBm','Quality','Security','Band','Channel','Frequency','Ping','Ports','Casambi','Raw'];return '<?xml version="1.0" encoding="UTF-8" standalone="yes"?><table xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main" id="1" name="DjungleScanTable" displayName="DjungleScanTable" ref="'+ref+'" headerRowCount="1" totalsRowShown="0"><autoFilter ref="'+ref+'"/><tableColumns count="14">'+cols.map(function(c,i){return '<tableColumn id="'+(i+1)+'" name="'+c+'"/>';}).join('')+'</tableColumns><tableStyleInfo name="TableStyleMedium4" showFirstColumn="0" showLastColumn="0" showRowStripes="1" showColumnStripes="0"/></table>';}
function crc32(buf){let t=window.crcTable;if(!t){t=[];for(let n=0;n<256;n++){let c=n;for(let k=0;k<8;k++)c=((c&1)?(0xedb88320^(c>>>1)):(c>>>1));t[n]=c>>>0;}window.crcTable=t;}let c=0xffffffff;for(let i=0;i<buf.length;i++)c=t[(c^buf[i])&255]^(c>>>8);return (c^0xffffffff)>>>0;}
function u16(a,v){a.push(v&255,(v>>>8)&255);}function u32(a,v){a.push(v&255,(v>>>8)&255,(v>>>16)&255,(v>>>24)&255);}function bytes(s){return Array.from(new TextEncoder().encode(s));}
function zipBuild(files){let out=[],central=[],off=0;const zt=0,zd=((2026-1980)<<9)|(1<<5)|1;files.forEach(function(f){let name=bytes(f.name),data=bytes(f.data),crc=crc32(data),local=[];u32(local,0x04034b50);u16(local,20);u16(local,0);u16(local,0);u16(local,zt);u16(local,zd);u32(local,crc);u32(local,data.length);u32(local,data.length);u16(local,name.length);u16(local,0);let rec=local.concat(name,data);out=out.concat(rec);let cen=[];u32(cen,0x02014b50);u16(cen,20);u16(cen,20);u16(cen,0);u16(cen,0);u16(cen,zt);u16(cen,zd);u32(cen,crc);u32(cen,data.length);u32(cen,data.length);u16(cen,name.length);u16(cen,0);u16(cen,0);u16(cen,0);u16(cen,0);u32(cen,0);u32(cen,off);central=central.concat(cen,name);off+=rec.length;});let cdStart=out.length;out=out.concat(central);let end=[];u32(end,0x06054b50);u16(end,0);u16(end,0);u16(end,files.length);u16(end,files.length);u32(end,central.length);u32(end,cdStart);u16(end,0);return new Uint8Array(out.concat(end));}
function xlsxBlob(type,text){let rows=xRows(type,text).length;let files=[{name:'[Content_Types].xml',data:'<?xml version="1.0" encoding="UTF-8"?><Types xmlns="http://schemas.openxmlformats.org/package/2006/content-types"><Default Extension="rels" ContentType="application/vnd.openxmlformats-package.relationships+xml"/><Default Extension="xml" ContentType="application/xml"/><Override PartName="/docProps/app.xml" ContentType="application/vnd.openxmlformats-officedocument.extended-properties+xml"/><Override PartName="/docProps/core.xml" ContentType="application/vnd.openxmlformats-package.core-properties+xml"/><Override PartName="/xl/workbook.xml" ContentType="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet.main+xml"/><Override PartName="/xl/worksheets/sheet1.xml" ContentType="application/vnd.openxmlformats-officedocument.spreadsheetml.worksheet+xml"/><Override PartName="/xl/tables/table1.xml" ContentType="application/vnd.openxmlformats-officedocument.spreadsheetml.table+xml"/><Override PartName="/xl/styles.xml" ContentType="application/vnd.openxmlformats-officedocument.spreadsheetml.styles+xml"/></Types>'},{name:'_rels/.rels',data:'<?xml version="1.0" encoding="UTF-8"?><Relationships xmlns="http://schemas.openxmlformats.org/package/2006/relationships"><Relationship Id="rId1" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/officeDocument" Target="xl/workbook.xml"/><Relationship Id="rId2" Type="http://schemas.openxmlformats.org/package/2006/relationships/metadata/core-properties" Target="docProps/core.xml"/><Relationship Id="rId3" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/extended-properties" Target="docProps/app.xml"/></Relationships>'},{name:'xl/workbook.xml',data:'<?xml version="1.0" encoding="UTF-8"?><workbook xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main" xmlns:r="http://schemas.openxmlformats.org/officeDocument/2006/relationships"><sheets><sheet name="Djungle Scan" sheetId="1" r:id="rId1"/></sheets></workbook>'},{name:'xl/_rels/workbook.xml.rels',data:'<?xml version="1.0" encoding="UTF-8"?><Relationships xmlns="http://schemas.openxmlformats.org/package/2006/relationships"><Relationship Id="rId1" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/worksheet" Target="worksheets/sheet1.xml"/><Relationship Id="rId2" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/styles" Target="styles.xml"/></Relationships>'},{name:'xl/worksheets/_rels/sheet1.xml.rels',data:'<?xml version="1.0" encoding="UTF-8"?><Relationships xmlns="http://schemas.openxmlformats.org/package/2006/relationships"><Relationship Id="rId1" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/table" Target="../tables/table1.xml"/></Relationships>'},{name:'xl/styles.xml',data:'<?xml version="1.0" encoding="UTF-8" standalone="yes"?><styleSheet xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main"><numFmts count="0"/><fonts count="4"><font><sz val="10"/><name val="Consolas"/></font><font><b/><sz val="16"/><color rgb="FF20E69A"/><name val="Consolas"/></font><font><b/><color rgb="FF91F36D"/><name val="Consolas"/></font><font><b/><color rgb="FFFFFFFF"/><name val="Consolas"/></font></fonts><fills count="4"><fill><patternFill patternType="none"/></fill><fill><patternFill patternType="gray125"/></fill><fill><patternFill patternType="solid"><fgColor rgb="FF06251E"/><bgColor indexed="64"/></patternFill></fill><fill><patternFill patternType="solid"><fgColor rgb="FF063D32"/><bgColor indexed="64"/></patternFill></fill></fills><borders count="2"><border><left/><right/><top/><bottom/><diagonal/></border><border><left style="thin"><color rgb="FF1DDCA8"/></left><right style="thin"><color rgb="FF1DDCA8"/></right><top style="thin"><color rgb="FF1DDCA8"/></top><bottom style="thin"><color rgb="FF1DDCA8"/></bottom><diagonal/></border></borders><cellStyleXfs count="1"><xf numFmtId="0" fontId="0" fillId="0" borderId="0"/></cellStyleXfs><cellXfs count="4"><xf numFmtId="0" fontId="0" fillId="0" borderId="0" xfId="0"/><xf numFmtId="0" fontId="1" fillId="2" borderId="1" xfId="0" applyFont="1" applyFill="1" applyBorder="1"/><xf numFmtId="0" fontId="2" fillId="2" borderId="1" xfId="0" applyFont="1" applyFill="1" applyBorder="1"/><xf numFmtId="0" fontId="3" fillId="3" borderId="1" xfId="0" applyFont="1" applyFill="1" applyBorder="1"/></cellXfs><cellStyles count="1"><cellStyle name="Normal" xfId="0" builtinId="0"/></cellStyles><dxfs count="0"/><tableStyles count="1" defaultTableStyle="TableStyleMedium4" defaultPivotStyle="PivotStyleLight16"/></styleSheet>'},{name:'xl/worksheets/sheet1.xml',data:sheetXml(type,text)},{name:'xl/tables/table1.xml',data:tableXml(rows)}];return new Blob([zipBuild(files)],{type:'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet'});}
function xFile(type){return 'casambi_jungle_'+String(type||'scan').replace(/[^A-Za-z0-9_-]/g,'_')+'_'+new Date().toISOString().replace(/[:.]/g,'_')+'.xlsx';}
function downloadXlsx(type,text){let a=document.createElement('a');a.href=URL.createObjectURL(xlsxBlob(type,text));a.download=xFile(type);a.click();setTimeout(function(){URL.revokeObjectURL(a.href)},1200);}
async function exportXlsxSmb(type,text){let blob=xlsxBlob(type,text);let buf=await blob.arrayBuffer();let bin='';let arr=new Uint8Array(buf);for(let i=0;i<arr.length;i++)bin+=String.fromCharCode(arr[i]);let b64=btoa(bin);let name=xFile(type);try{let r=await fetch('/api/export-b64-smb?name='+encodeURIComponent(name)+'&data='+encodeURIComponent(b64),{cache:'no-store'});let j=await r.json();alert(j.ok?'XLSX SMB Export gespeichert: '+j.file:'XLSX SMB Fehler: '+j.error);}catch(e){alert('XLSX SMB Fehler: '+e);}}
async function exportTxtSmb(type,text){if(!text||!text.trim()){alert('Kein Export-Text vorhanden.');return;}let r=await fetch('/api/export-scan-smb?type='+encodeURIComponent(type)+'&text='+encodeURIComponent(text),{cache:'no-store'});let j=await r.json();alert(j.ok?'TXT SMB Export gespeichert: '+j.file:'TXT SMB Fehler: '+j.error);}
function exportTerminalTxtSmb(type,id){exportTxtSmb(type,document.getElementById(id).textContent||'');}function exportTerminalXlsxSmb(type,id){exportXlsxSmb(type,document.getElementById(id).textContent||'');}
...
<div class='hero'>
  ...
  <div class='sub'>${esc(c.casambiNetworkName.ifBlank { "Bridge Control Center" })} - powered by Sambesi - v0.10.5</div>
...
<section class='hero card wide glassHero'><h1>Scan History Pro</h1><p class='muted'>Letzte 10 Web-Scans mit echter XLSX-Tabelle, SMB Export, Schnellfiltern, Sortierung, Delete, Copy und TXT Export.</p></section>
<section class='card wide historyTools'>
  ...
  <button class='btn ghost' onclick='exportFilteredTxtSmb()'>EXPORT FILTERED SMB</button>
  <button class='btn ghost' onclick='exportFilteredXlsxSmb()'>EXPORT FILTERED XLSX SMB</button>
  <button class='btn ghost' onclick='downloadFilteredXlsx()'>DOWNLOAD FILTERED XLSX</button>
  <button class='btn ghost' onclick='setQuickFilter("all")'>ALL</button><button class='btn ghost' onclick='setQuickFilter("wifi")'>WiFi</button><button class='btn ghost' onclick='setQuickFilter("network")'>Network</button><button class='btn ghost' onclick='setQuickFilter("bluetooth")'>Bluetooth</button><button class='btn ghost' onclick='setQuickFilter("casambi")'>Casambi</button><button class='btn ghost' onclick='setQuickFilter("wpa2")'>WPA2</button><button class='btn ghost' onclick='setQuickFilter("enterprise")'>Enterprise</button><button class='btn ghost' onclick='setQuickFilter("open")'>Open</button><button class='btn ghost' onclick='setQuickFilter("excellent")'>Strong</button><button class='btn ghost' onclick='setQuickFilter("weak")'>Weak</button><button class='btn ghost' onclick='setQuickFilter("host=")'>Hosts</button><button class='btn ghost' onclick='expandHistory()'>EXPAND ALL</button><button class='btn ghost' onclick='collapseHistory()'>COLLAPSE ALL</button><button class='btn danger' onclick='clearHistory()'>CLEAR HISTORY</button>
</section>
...
function u16(a,v){a.push(v&255,(v>>>8)&255);}function u32(a,v){a.push(v&255,(v>>>8)&255,(v>>>16)&255,(v>>>24)&255);}function bytes(s){return Array.from(new TextEncoder().encode(s));}
function zipBuild(files){let out=[],central=[],off=0;const zt=0,zd=((2026-1980)<<9)|(1<<5)|1;files.forEach(function(f){let name=bytes(f.name),data=bytes(f.data),crc=crc32(data),local=[];u32(local,0x04034b50);u16(local,20);u16(local,0);u16(local,0);u16(local,zt);u16(local,zd);u32(local,crc);u32(local,data.length);u32(local,data.length);u16(local,name.length);u16(local,0);let rec=local.concat(name,data);out=out.concat(rec);let cen=[];u32(cen,0x02014b50);u16(cen,20);u16(cen,20);u16(cen,0);u16(cen,0);u16(cen,zt);u16(cen,zd);u32(cen,crc);u32(cen,data.length);u32(cen,data.length);u16(cen,name.length);u16(cen,0);u16(cen,0);u16(cen,0);u16(cen,0);u32(cen,0);u32(cen,off);central=central.concat(cen,name);off+=rec.length;});let cdStart=out.length;out=out.concat(central);let end=[];u32(end,0x06054b50);u16(end,0);u16(end,0);u16(end,files.length);u16(end,files.length);u32(end,central.length);u32(end,cdStart);u16(end,0);return new Uint8Array(out.concat(end));}
function xlsxBlob(type,text){let rows=xRows(type,text).length;let files=[{name:'[Content_Types].xml',data:'<?xml version="1.0" encoding="UTF-8"?><Types xmlns="http://schemas.openxmlformats.org/package/2006/content-types"><Default Extension="rels" ContentType="application/vnd.openxmlformats-package.relationships+xml"/><Default Extension="xml" ContentType="application/xml"/><Override PartName="/docProps/app.xml" ContentType="application/vnd.openxmlformats-officedocument.extended-properties+xml"/><Override PartName="/docProps/core.xml" ContentType="application/vnd.openxmlformats-package.core-properties+xml"/><Override PartName="/xl/workbook.xml" ContentType="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet.main+xml"/><Override PartName="/xl/worksheets/sheet1.xml" ContentType="application/vnd.openxmlformats-officedocument.spreadsheetml.worksheet+xml"/><Override PartName="/xl/tables/table1.xml" ContentType="application/vnd.openxmlformats-officedocument.spreadsheetml.table+xml"/><Override PartName="/xl/styles.xml" ContentType="application/vnd.openxmlformats-officedocument.spreadsheetml.styles+xml"/></Types>'},{name:'_rels/.rels',data:'<?xml version="1.0" encoding="UTF-8"?><Relationships xmlns="http://schemas.openxmlformats.org/package/2006/relationships"><Relationship Id="rId1" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/officeDocument" Target="xl/workbook.xml"/><Relationship Id="rId2" Type="http://schemas.openxmlformats.org/package/2006/relationships/metadata/core-properties" Target="docProps/core.xml"/><Relationship Id="rId3" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/extended-properties" Target="docProps/app.xml"/></Relationships>'},{name:'xl/workbook.xml',data:'<?xml version="1.0" encoding="UTF-8"?><workbook xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main" xmlns:r="http://schemas.openxmlformats.org/officeDocument/2006/relationships"><sheets><sheet name="Djungle Scan" sheetId="1" r:id="rId1"/></sheets></workbook>'},{name:'xl/_rels/workbook.xml.rels',data:'<?xml version="1.0" encoding="UTF-8"?><Relationships xmlns="http://schemas.openxmlformats.org/package/2006/relationships"><Relationship Id="rId1" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/worksheet" Target="worksheets/sheet1.xml"/><Relationship Id="rId2" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/styles" Target="styles.xml"/></Relationships>'},{name:'xl/worksheets/_rels/sheet1.xml.rels',data:'<?xml version="1.0" encoding="UTF-8"?><Relationships xmlns="http://schemas.openxmlformats.org/package/2006/relationships"><Relationship Id="rId1" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/table" Target="../tables/table1.xml"/></Relationships>'},{name:'xl/styles.xml',data:'<?xml version="1.0" encoding="UTF-8"?><styleSheet xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main"><fonts count="4"><font><sz val="10"/><name val="Consolas"/></font><font><b/><sz val="16"/><color rgb="FF20E69A"/><name val="Consolas"/></font><font><b/><color rgb="FF91F36D"/><name val="Consolas"/></font><font><b/><color rgb="FFFFFFFF"/><name val="Consolas"/></font></fonts><fills count="4"><fill><patternFill patternType="none"/></fill><fill><patternFill patternType="gray125"/></fill><fill><patternFill patternType="solid"><fgColor rgb="FF06251E"/></patternFill></fill><fill><patternFill patternType="solid"><fgColor rgb="FF063D32"/></patternFill></fill></fills><borders count="2"><border/><border><left style="thin"><color rgb="FF1DDCA8"/></left><right style="thin"><color rgb="FF1DDCA8"/></right><top style="thin"><color rgb="FF1DDCA8"/></top><bottom style="thin"><color rgb="FF1DDCA8"/></bottom></border></borders><cellXfs count="4"><xf/><xf fontId="1" fillId="2" borderId="1" applyFont="1" applyFill="1"/><xf fontId="2" fillId="2" borderId="1" applyFont="1" applyFill="1"/><xf fontId="3" fillId="3" borderId="1" applyFont="1" applyFill="1"/></cellXfs></styleSheet>'},{name:'xl/worksheets/sheet1.xml',data:sheetXml(type,text)},{name:'xl/tables/table1.xml',data:tableXml(rows)}];return new Blob([zipBuild(files)],{type:'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet'});}
function xFile(type){return 'casambi_jungle_'+String(type||'scan').replace(/[^A-Za-z0-9_-]/g,'_')+'_'+new Date().toISOString().replace(/[:.]/g,'_')+'.xlsx';}
function downloadXlsx(type,text){let a=document.createElement('a');a.href=URL.createObjectURL(xlsxBlob(type,text));a.download=xFile(type);a.click();setTimeout(function(){URL.revokeObjectURL(a.href)},1200);}
async function exportXlsxSmb(type,text){let blob=xlsxBlob(type,text);let buf=await blob.arrayBuffer();let bin='';let arr=new Uint8Array(buf);for(let i=0;i<arr.length;i++)bin+=String.fromCharCode(arr[i]);let b64=btoa(bin);let name=xFile(type);try{let r=await fetch('/api/export-b64-smb?name='+encodeURIComponent(name)+'&data='+encodeURIComponent(b64),{cache:'no-store'});let j=await r.json();alert(j.ok?'XLSX SMB Export gespeichert: '+j.file:'XLSX SMB Fehler: '+j.error);}catch(e){alert('XLSX SMB Fehler: '+e);}}
async function exportTxtSmb(type,text){if(!text||!text.trim()){alert('Kein Export-Text vorhanden.');return;}let r=await fetch('/api/export-scan-smb?type='+encodeURIComponent(type)+'&text='+encodeURIComponent(text),{cache:'no-store'});let j=await r.json();alert(j.ok?'TXT SMB Export gespeichert: '+j.file:'TXT SMB Fehler: '+j.error);}
...
function exportHistoryTxtSmb(index){let h=getHistory()[index];if(!h)return;exportTxtSmb(String(h.type||'scan'),String(h.text||''));}
function exportHistoryXlsx(index){let h=getHistory()[index];if(!h)return;downloadXlsx(String(h.type||'scan'),String(h.text||''));}
function exportHistoryXlsxSmb(index){let h=getHistory()[index];if(!h)return;exportXlsxSmb(String(h.type||'scan'),String(h.text||''));}
...
');}
function exportFilteredTxtSmb(){exportTxtSmb('history_filtered',filteredText());}
function exportFilteredXlsxSmb(){exportXlsxSmb('history_filtered',filteredText());}
function downloadFilteredXlsx(){downloadXlsx('history_filtered',filteredText());}
...
').filter(Boolean);const sum=quickSummary(lines);const sorted=sortLines(lines);const body=sorted.map(lineCard).join('');let best=sum.best?(' · Best '+sum.best+' dBm'):'';return "<details class='historyItem'><summary><span>"+escHtml(h.time)+"</span><b>"+escHtml(h.type)+"</b><em>"+lines.length+" Zeilen · WiFi "+sum.wifi+" · Hosts "+sum.net+" · BLE "+sum.ble+" · Casambi "+sum.cas+best+"</em><button onclick='event.preventDefault();event.stopPropagation();deleteHistory("+o.i+")'>🗑 Delete</button><button onclick='event.preventDefault();event.stopPropagation();copyHistory("+o.i+")'>Copy</button><button onclick='event.preventDefault();event.stopPropagation();exportHistory("+o.i+")'>TXT</button><button onclick='event.preventDefault();event.stopPropagation();exportHistoryTxtSmb("+o.i+")'>SMB</button><button onclick='event.preventDefault();event.stopPropagation();exportHistoryXlsx("+o.i+")'>XLSX</button><button onclick='event.preventDefault();event.stopPropagation();exportHistoryXlsxSmb("+o.i+")'>XLSX SMB</button></summary><div class='historyTree'>"+body+"</div></details>";}).join('');}
loadHistory();
</script>
""")
