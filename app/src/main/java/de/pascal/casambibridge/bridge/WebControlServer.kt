...
<section class='card wide scannerGrid'>
  ...
  <div class='scannerPanel'>
    ...
    <pre id='wifiTerminal' class='terminal smallTerminal'>Bereit. Android 8.1 kann WiFi ScanResults leer liefern.</pre>
  </div>
  <div class='scannerPanel'>
    ...
    <pre id='bleTerminal' class='terminal smallTerminal'>Bereit.</pre>
  </div>
</section>
<script>
...
function clearTerminal(id,progress){document.getElementById(id).textContent='Bereit.';document.getElementById(progress).style.width='0%'; if(id==='bleTerminal')seenBle.clear(); if(id==='wifiTerminal')seenWifi.clear();}
...
""")
    ...
    private fun scanWifiLiveJson(params: Map<String,String>): String {
        ...
        Thread.sleep(350)
        val arr = org.json.JSONArray()
        runCatching { wifi.scanResults }.getOrDefault(emptyList()).map { r ->
        ...
        }.filter { filter.isBlank() || it.lowercase().contains(filter) }.sortedByDescending { it.substringAfter("rssi=").substringBefore(" ").trim().toIntOrNull() ?: -999 }.forEach { arr.put(it) }
...
<div class='hero'>
  ...
  <div class='sub'>${esc(c.casambiNetworkName.ifBlank { "Bridge Control Center" })} - powered by Sambesi - v0.8.3</div>
